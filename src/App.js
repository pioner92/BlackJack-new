...
import cartt from "./components/Images"
import "./App.css";
import Footer from "./components/Footer.jsx";
import Money from "./components/Money.jsx";
import Logo from "./components/Logo.jsx";
import ResultAlert from "./components/ResultAlert.jsx";
import BlockCarts from "./components/BlockCarts.jsx";
...
function App() {
    ...
    useEffect(() => {                           // Прослушка счета бота и остановка его ходов и вывод результата
        ...
        if (score_bot === score && botStop) {       // Если счета у игрока и бота совпадают - Ничья
            setResult('Ничья')
            return ;
        }
        if (score_bot === 21) {                    // Если у бота 21 Он победил, игрок проиграл
            setBotStop(true)
            losing()
            return;
        }
        ...
        if (botStop && score < score_bot && score_bot <= 21) {  // Если у игрока меньше очков чем у бота = проиграл
            losing()
            return;
        }
    ...
    useEffect(() => {                       // Прослушка счета игрока, и изминение наличия денег
        if (score === 21) {                      // Если игрок набрал 21 , Победа
            victory()
        }
